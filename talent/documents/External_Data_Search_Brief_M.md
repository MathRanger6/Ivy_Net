...
**How to use this document:** Paste this entire document into GPT-4, GPT-5, Scholar GPT, Claude, or another LLM, then add a request such as: *"Given the research, data, and criteria below, please suggest specific publicly available datasets, data repositories, or published papers that provide data meeting these criteria, so we can test whether our main finding replicates in other settings. Include names of datasets, where to access them, and why they fit."*
