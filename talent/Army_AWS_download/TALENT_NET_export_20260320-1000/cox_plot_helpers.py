...
def plot_competing_risks_cif_bars(
    ...
    q_labels = [f"Q{i+1}" for i in range(len(labels))]
    ...
    promo_palette = _build_hex_gradient(promo_palette or ["#d7e6f5", "#1f5aa6"], len(q_labels))
    attr_palette = _build_hex_gradient(attr_palette or ["#fde2cf", "#c0581a"], len(q_labels))

    # Build color maps aligned to ordered groups so bar colors match CR curves
    ...
    promo_df = pd.DataFrame({"bin": q_labels, "cif": promo_vals})
    attr_df = pd.DataFrame({"bin": q_labels, "cif": attr_vals})
    promo_df["bin"] = pd.Categorical(promo_df["bin"], categories=q_labels, ordered=True)
    attr_df["bin"] = pd.Categorical(attr_df["bin"], categories=q_labels, ordered=True)
    ...
    ax1.set_xlabel(None)  # Removed for paper
    ax1.set_xticks(x)
    ...
    ax2.set_xlabel(None)  # Removed for paper
    ax2.set_xticks(x)
