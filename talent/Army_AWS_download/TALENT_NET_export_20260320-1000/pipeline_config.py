...
Import this file in the notebook to access all settings:
    ...
    from py_pipeline_config import *

Or import specific items:

    from py_pipeline_config import CELL0, CELL1, PLOT_CONFIG, COLUMN_CONFIG
...
#     (e.g. [POOL_MINUS_MEAN_SNR_COL] or [TB_RATIO_SNR_COL, POOL_MINU_MEAN_SNR)COL])
#   - combined_effect_base: optional raw column name for Cell 12.6A combined-effect plot; must be
#     in quadratic bases.  If omitted, quadratic_bases[0] is used.
...
#   - winsorize: optional dict to override STANDARDIZE_CONFIG['winsorize'] (e.g. {'enabled': False} to trun off)
...
#   - interaction_log_xy: optional bool - use log scale a and y axes (only when both axes >0, e.g. raw)
#   - interaction_slice_x_at: optional 'max' or percentile(e.g. 99)- ID slice: fix x at top, plot HR vs y
...
def expand_run_profile(run_scale, run_profile):
    ...
    # Override winsorize from RUN_PROFILE (e.g. {'enabled': Flae} to turn off for this run)
    if 'winsorize' in run_profile:
        _cfg.STANDARDIZE_CONFIG['winsorize'] = {**_cfg.STANDARDIZE_CONFIG.get('winsorize'), **run_profile['winsorize']}
    # Override interaction plot options from RUN_PROFILE (overrid key -> INTERACTION_CONFIG key)
    ...
    _combo_base_raw = run_profile.get('combined_effect_bas') or (quadratic_bases[0] if quadratic_bases else None)
...
INTERACTION_CONFIG = {
    ...
    # Log scale: log_z = hazard ratio (z-axis / color); log_xy = x and y axes (only when both axes > 0, e.g. raw)
    'log_z': False,
    'log_xy': False,
