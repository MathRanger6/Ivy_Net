...
# --- div_name + model_columns ---
# Any other time-varying categorical for Cox: append to EXTRA_TIME_VARYING_COLS (or model_list), add an entry
...
# These are carried into df_cox (Cell 10) and the Cox model; div_* dumies appear in top-15 signal ratios (Cell 12.4)
...
# Dummies required for categoricals (div_name, yg, etc.); base config has create dummies False
...
# EXTRA_STATIC_COLS = ['yg']  #optional; then: COLUMN_CONFIG['model_static_cols'] += EXTRA_STATIC_COL

# Use explicit CR plots (TB ratio by div_name, etc.); below only when True; when False (default), automated plots from expand_run_profile()
...
if USE_OVERRIDE_CR_PLOTS:
    ...
    PLOT_CONFIG['plots'] = [
        ...
        # TB ratio, 1 bin, grouped by division (div_name in model -> auto in time_varying_cols -> in df_cox)
        {
            **RUN_CR_PLOT_DEFAULTS,
            'variable': _cr_var_tb,
            'n_bins': 1,
...
_model_tv = COLUMN_CONFIG.get('base_time_varying_cols', [])
