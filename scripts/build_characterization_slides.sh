...
#   slides/CHAR_PD16_HAND.pptx
#
# Usage (repo root):
#   ./scripts/build_characterization_slides.sh
#       → figures only (default) — team L_C + θ(K/N); updates PNG/CSV; no .pptx touched
#
...
#       → parallel *_pd16.png copies (same team L_C + θ(K/N); suffix for side-by-side)
#
# Hand deck workflow:
#   1. Format equations/layout once in slides/CHAR_PD16_HAND.pptx
...
#   5. After hand edits: export deck → slides/CHAR_PD16_HAND/SlideN.jpeg
...
AUTO="$SLIDES/auto"
HAND="$SLIDES/CHAR_PD16_HAND.pptx"
MODE="figures"
MERGE=1
PD16=0
...
# Default gallery: team L_C + θ(K/N) on sim ability draw (Aug 2026 — LOO L_C parked).
export GALLERY_LC_MODE="${GALLERY_LC_MODE:-team_smooth}"
export GALLERY_THETA_MODE="${GALLERY_THETA_MODE:-k_over_n}"
export GALLERY_HERO_SEED="${GALLERY_HERO_SEED:-42}"

if [[ "$PD16" -eq 1 ]]; then
  export GALLERY_OUTPUT_SUFFIX=_pd16
  echo "PD16 parallel suffix: *_pd16.png (baseline names still team L_C + θ(K/N))"
fi

echo "Gallery modes: GALLERY_LC_MODE=$GALLERY_LC_MODE GALLERY_THETA_MODE=$GALLERY_THETA_MODE GALLERY_HERO_SEED=$GALLERY_HERO_SEED"
...
_run_figures() {
  ...
  echo ">> θ figures ..."
  GALLERY_THETA_MODE=preset "$PYTHON" sports/scripts/theta_kn_sweep_diagnostic.py
  GALLERY_THETA_MODE=k_over_n "$PYTHON" sports/scripts/theta_kn_sweep_diagnostic.py
