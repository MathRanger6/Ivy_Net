#!/bin/bash
# Convert only .md files in current_documents/ to PDF
# Uses generate_pdf_playwright.sh for each file
# PDFs are created in current_documents/
# Usage: ./convert_current_documents_md_to_pdf.sh [--keep-html]
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --keep-html)
            ...
            ;;
        -h|--help)
            echo "Usage: ./convert_current_documents_md_to_pdf.sh [--keep-html]"
            exit 0
            ;;
        *)
...
# Only look in current_documents
CURRENT_DOCS_DIR="$WORKSPACE_DIR/current_documents"
if [ ! -d "$CURRENT_DOCS_DIR" ]; then
    echo "❌ Error: current_documents directory not found at: $CURRENT_DOCS_DIR"
    exit 1
fi

echo "🔍 Finding all .md files in: $CURRENT_DOCS_DIR"
MD_FILES=$(find "$CURRENT_DOCS_DIR" -type f -name "*.md" | sort)
...
if [ "$TOTAL_FILES" -eq 0 ]; then
    echo "⚠️  No .md files found in current_documents"
    exit 0
fi
