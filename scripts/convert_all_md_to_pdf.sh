#!/bin/bash
# Script to convert all .md files in Cursor Workspace PDE to PDF
# Uses generate_pdf_playwright.sh for each file
# PDFs are created in current_documents/
# Usage: ./convert_all_md_to_pdf.sh [--keep-html]
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --keep-html)
            ...
            ;;
        -h|--help)
            echo "Usage: ./convert_all_md_to_pdf.sh [--keep-html]"
            exit 0
            ;;
        *)
...
# Find all .md files in workspace and subdirectories
# Exclude obsolete_documents, .ipynb_checkpoints, 1-Various_PDE_and_Chat_stuff, and other subdirectories
echo "🔍 Finding all .md files in: $WORKSPACE_DIR (excluding obsolete_documents, .ipynb_checkpoints, 1-Various_PDE_and_Chat_stuff, and other subdirectories)"
MD_FILES=$(find "$WORKSPACE_DIR" -type f -name "*.md" \
    -not -path "*/obsolete_documents/*" \
    -not -path "*/.ipynb_checkpoints/*" \
    -not -path "*/1-Various_PDE_and_Chat_stuff/*" \
    -not -path "*/obsolete_files/*" \
    -not -path "*/cox_model/*" | sort)
...
if [ "$TOTAL_FILES" -eq 0 ]; then
    echo "⚠️  No .md files found in workspace"
    exit 0
fi
...
# Create current_documents directory if it doesn't exist
CURRENT_DOCS_DIR="$WORKSPACE_DIR/current_documents"
if [ ! -d "$CURRENT_DOCS_DIR" ]; then
    mkdir -p "$CURRENT_DOCS_DIR"
    echo "📁 Created directory: current_documents"
    echo ""
fi
...
while IFS= read -r md_file; do
    ...
    # Save PDF in current_documents directory instead of same directory as .md file
...
echo "   📁 PDFs saved to: current_documents/"
echo ""
