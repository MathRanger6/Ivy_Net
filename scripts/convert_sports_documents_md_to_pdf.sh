...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --keep-html)
            ...
            ;;
        -h|--help)
            echo "Usage: ./convert_sports_documents_md_to_pdf.sh [--keep-html]"
            exit 0
            ;;
        *)
...
echo "🔍 Finding all .md files in: $SPORTS_DOCS_DIR"
MD_FILES=$(find "$SPORTS_DOCS_DIR" -type f -name "*.md" | sort)
...
if [ "$TOTAL_FILES" -eq 0 ]; then
    echo "⚠️  No .md files found in current_documents/sports_documents"
    exit 0
fi
