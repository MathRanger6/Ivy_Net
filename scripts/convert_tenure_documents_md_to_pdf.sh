#!/bin/bash
# Convert only .md files in current_documents/tenure_documents/ to PDF
# Uses generate_pdf_playwright.sh for each file
# PDFs are created in current_documents/tenure_documents/
...
# Only look in current_documents/tenure_documents
TENURE_DOCS_DIR="$WORKSPACE_DIR/current_documents/tenure_documents"
if [ ! -d "$TENURE_DOCS_DIR" ]; then
    echo "❌ Error: tenure_documents directory not found at: $TENURE_DOCS_DIR"
    exit 1
fi
...
while IFS= read -r md_file; do
    ...
    # Save PDF in tenure_documents directory
...
echo "   📁 PDFs saved to: current_documents/tenure_documents/"
echo ""
