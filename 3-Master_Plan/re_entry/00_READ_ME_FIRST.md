...
**Last synced:** 2026-07-27 — docs 01–03 + slide + sim OPORD; **ρ** assignment re-entry via `540_*`.
...
**#1 confusion (Charles lock):** Environment **`L_net = B − D`** ≠ selection **`S_i`** (Alex score). Locked in doc **02** and [`../BINDING_Selection_is_its_own_step.md`](../BINDING_Selection_is_its_own_step.md).
...
- [`Model.pdf`](Model.pdf) / [`Model.pptx`](Model.pptx) — **your one-slide story** (two-step pipeline, unified **S_i**, Alex v1 **(B−D)=−L_C**, empirical Naïve/Hero, knockout **λ=0**)
...
**Alex v1 minimal model (Jul 2026):** empirical hero + talent baseline; generative **assign → S_i → top K** with **λ=0** vs **S_i = A_i − λ·L_C** knockout; side-by-side figure + limitation sentence. **ρ** (assignment assortativity) ablation is optional follow-up — see [`../../sports/540_READ_ME_SIM.md`](../../sports/540_READ_ME_SIM.md) and [`model_OPORD.md`](model_OPORD.md).
