...
def run_scenario(sc: Scenario) -> dict:
    ...
    interior_peak = bool(0 <= peak_idx < y_valid.size - 1)
    moderate_downturn = bool(interior_peak and tail_drop_frac >= 0.05)
...
def grouped_candidates(stage2_rows: list[dict]) -> pd.DataFrame:
    ...
    grouped = grouped.sort_values(
        ["moderate_stable", "mean_tail_drop_frac", "interior_seed_count"],
...
A seed is marked `moderate_downturn=True` when:

- max promotion probability is in an interior bin; and
- final bin is at least 5% below that peak.

A grouped setting is marked `moderate_stable=True` when:

- at least 60% of seeds satisfy `moderate_downturn`; and
- mean tail drop is at least 5%.
