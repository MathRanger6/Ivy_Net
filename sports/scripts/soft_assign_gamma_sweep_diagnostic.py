#!/usr/bin/env python3
"""γ × λ sweep on soft assign (overlapping rosters) — parallel to sort-and-chop grid.

Same league draw + soft assignment at fixed ρ per γ row; sweep λ in score.
Team L_C + θ(K/N) under current gallery defaults. No λ_crit ≈ 4/γ claim here.

Run (repo root):
  python sports/scripts/soft_assign_gamma_sweep_diagnostic.py

Outputs (HEROs_and_PASSes/soft_assign_lambda/):
  GAMMA_sweep_soft_assign_lambda_curves.png
  GAMMA_sweep_soft_assign_lambda_curves_key_arms.png
  GAMMA_sweep_soft_assign_summary.csv
  GAMMA_sweep_soft_assign_meta.json
"""
...
from hero_gallery_paths import SOFT_ASSIGN_LAMBDA, ensure_hero_dirs

OUT = SOFT_ASSIGN_LAMBDA
PNG = OUT / "GAMMA_sweep_soft_assign_lambda_curves.png"
PNG_KEY = OUT / "GAMMA_sweep_soft_assign_lambda_curves_key_arms.png"
CSV = OUT / "GAMMA_sweep_soft_assign_summary.csv"
META_JSON = OUT / "GAMMA_sweep_soft_assign_meta.json"
...
def _plot_gamma_panels(
    ...
    *,
    ...
    for ax, gamma in zip(axes, GAMMA_GRID):
        ...
        ax.set_title(
            rf"$\gamma={gamma:g}$  —  soft assign, overlapping rosters",
            fontsize=12,
            loc="left",
            pad=8,
        )
    ...
    subtitle = (
        rf"Soft assign $\rho={rho_fixed:g}$, $\sigma={sigma:g}$ | "
    ...
    fig.suptitle(
        rf"$\gamma$ sweep — congestion in score, overlapping assign ({PRESET}){suptitle_extra}",
        fontsize=13,
        y=0.995,
    )
    ...
    fig.subplots_adjust(left=0.1, right=0.98, top=0.93, bottom=0.13, hspace=0.32)
...
def main() -> None:
    ...
    for gamma in GAMMA_GRID:
        print(f"γ={gamma:g} — soft assign ρ={rho_fixed:g}, drawing roster ...")
    ...
    meta = {
        "diagnostic": "gamma_sweep_soft_assign",
        ...
        "note": "Overlap regime — no lambda_crit 4/gamma law; compare to sort_chop_lambda/ grid.",
    }
    ...
    build_figure(
        ...
        gallery_theta=gallery_theta,
        rho_fixed=rho_fixed,
        sigma=float(params.assignment_sigma),
    )
    print("Done.")
