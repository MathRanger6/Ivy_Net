...
def _plot_dh(summary: pd.DataFrame, out_path: Path) -> None:
    ...
    for ax, col, ylab, title in specs:
        ...
        ax.set_title(title)
        ax.grid(True, alpha=0.3)
    fig.suptitle("Grandchild ASSIGN — 2015 empirical PPM z · C=15 · ρ ∈ [0, 1]", fontsize=11)
    fig.tight_layout()
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def main() -> None:
    ...
    print(f"541 sweep: season={args.season} N={n} J={n_teams} C={c} reps={n_rep}")

    detail, summary, examples = _run_sweep(
        ability, c, rho_values, n_rep, args.seed
    )
