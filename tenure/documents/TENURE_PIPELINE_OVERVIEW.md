...
**HPC / large files:** Large artifacts (snapshots, JSONL, DBLP XML) are excluded from Git by design — see **`.gitignore`**. For cloning on UVA’s cluster, syncing those directories, and keeping paths aligned with Cell 0, use **`HPC_SETUP_CHECKLIST.md`** (same folder as this file: `tenure/documents/`).
...
### CELL 6A–6B — Entity resolution: OpenAlex (implemented) + incremental snapshot cache

**Primary path (in code):** `tenure_pipeline/openalex_resolver.py`, invoked from **`540`** CELL **6A** (institution + author IDs) and **6B** (publication counts by year). Outputs under `tenure_pipeline/openalex_*.json` / `*.jsonl` (see CELL 0 constants: `STAGE6_*`).
...
#### Cell 6B — CDH bulk snapshot (implemented) + incremental cache

Cell 6B uses the UVA Connected Data Hub **full OpenAlex** snapshot (`.csv.gz` shards under `~/cdh/OpenAlex1125/` on Rivanna). This avoids the OpenAlex API entirely for works-by-year — no rate limits, no HTTP calls. The snapshot is a static point-in-time dump, so extracted data never becomes stale.

**Incremental snapshot cache (`openalex_snapshot_cache.jsonl`):**
Each author's extracted works-by-year is persisted to this JSONL after their first scan. On subsequent runs, cached authors are served instantly — zero snapshot I/O. Only new authors (from adding schools) ever trigger another scan.

| Run | What happens |
|-----|-------------|
| First run (no cache) | Full scan of all PAA shards + pub2year (~3–5 hours); cache built simultaneously |
| Subsequent runs (all cached) | All authors served from cache in seconds |
| New schools added (partial cache) | Only new author IDs scanned; existing cache untouched |

**`build_openalex_cache.py` / `build_openalex_cache.slurm`:** Dedicated standalone script for the first-run cache build. Produces rich progress output (shard progress, ETA, hit rate) and writes both `openalex_snapshot_cache.jsonl` and `openalex_works_by_year.jsonl` incrementally — safe to interrupt and resume. Submit via `sbatch build_openalex_cache.slurm` from repo root.

**Cell 6B routing logic (auto-detected at runtime):**

| Situation | Route | Behavior |
|-----------|-------|----------|
| On Rivanna, snapshot accessible | Route 1 — Snapshot | Bulk scan for uncached authors; instant for cached |
| On Rivanna, all authors cached | Route 1 — Cache hit | Zero scan; output written from cache in seconds |
| Mac / no HPC, cache file exists (rsync'd) | Route 2 — Cache-only | All cached authors written; uncached → instructions printed |
| Mac / no HPC, `STAGE6B_API_FALLBACK=True` | Route 2 — Cache + API | Cached → instant; uncached → OpenAlex API (slow) |
| No snapshot, no cache, `STAGE6B_API_FALLBACK=True` | Route 3 — API only | One request/author; small exploratory runs only |
| No snapshot, no cache, `STAGE6B_API_FALLBACK=False` | Route 4 — Blocked | Prints rsync instructions; returns `[]` |

**`STAGE6B_API_FALLBACK` (Cell 0):** Default `False`. This is a **deliberate gate** — the API path must be explicitly enabled. It prevents accidentally falling through to slow rate-limited calls when the snapshot or cache is simply not mounted. Set `True` only for small exploratory runs.

**Coffee-shop / offline workflow (Route 2):**
1. On Rivanna: `sbatch build_openalex_cache.slurm` → builds cache + works file
2. `rsync` both `openalex_snapshot_cache.jsonl` and `openalex_works_by_year.jsonl` to local Mac
3. Run notebook locally — Cell 6B serves from cache instantly; Stages 7–9 run entirely offline
4. When new schools are added: SSH-submit `build_openalex_cache.slurm` remotely, rsync again

**gitignore:** Both `openalex_snapshot_cache.jsonl` and `openalex_works_by_year.jsonl` are gitignored (data files, not code). Transfer via rsync, not git.
...
tenure_pipeline/
  ...
  openalex_works_by_year.jsonl             ← CELL 6B: publication counts by year (gitignored — rsync only)
  openalex_snapshot_cache.jsonl            ← CELL 6B: incremental per-author cache (gitignored — rsync only)
  openalex_low_confidence.jsonl            ← 6A: MULTI / LOW / NONE for review
  build_openalex_cache.py                  ← standalone cache builder (first-run / incremental; run on Rivanna)

build_openalex_cache.slurm               ← Slurm job to run build_openalex_cache.py (12h, 32GB, 4 CPUs)
